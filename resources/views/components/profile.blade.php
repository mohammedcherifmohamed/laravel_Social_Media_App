  ...
  <section class="max-w-4xl mx-auto mt-20 mb-10 bg-white rounded-lg shadow p-0 overflow-hidden flex flex-col ">
    ...
    <div class="w-full flex flex-col items-center justify-center p-8 bg-gray-50">
      ...
      <div class="font-bold text-2xl text-center mt-2">{{Auth::user()->name}}</div>
  ...
  <div id="editProfileModal" class="hidden fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative" tabindex="-1" id="editProfileDialog">
      ...
      <form class="space-y-4" id="editProfileForm" onsubmit="return handleProfileSave(event)">
        <div class="flex flex-col items-center">
          <label for="editProfilePic" class="relative cursor-pointer group">
            <img id="editProfilePicPreview" src="https://randomuser.me/api/portraits/men/32.jpg" class="h-24 w-24 rounded-full object-cover border-2 border-blue-400"/>
            ...
            <input id="editProfilePic" type="file" accept="image/*" class="hidden" onchange="previewProfilePic(event)" />
          </label>
        ...
        <div>
          ...
          <input id="editUsername" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Enter new username" required />
        </div>
        <div>
          <label class="block text-sm font-medium mb-1" for="editPassword">Password <span class="text-red-500">*</span></label>
          <div class="relative">
            <input id="editPassword" type="password" minlength="6" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 pr-10" placeholder="Enter new password" required />
